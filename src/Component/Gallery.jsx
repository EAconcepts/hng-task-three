...
const Gallery = ({imageList, setImageList, searchResult}) => {
  ...
  return (
    <div className="w-full min-h-scree">
      <div className="w-full px-8">
        <Reorder
          className="w-full mt-10 grid grid-cols-3 lg:grid-cols-4 gap-1  rounded-lg border drop-shadow-lg"
        ...
        >
          {images?.map((image, index) => (
            <div key={index} className=" relative sm:w-72 sm:h-72 border">
              <img
                src={image.url}
                alt={`${image.tag} image`}
